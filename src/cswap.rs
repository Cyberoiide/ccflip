...
/// cswap's `slugify_email`: every character outside `[alnum._-]` becomes `_`.
/// Guessing only at `@` misses `first+tag@host` and friends, and the miss is
/// silent — the pinned profile simply never resolves.
fn slugify_email(email: &str) -> String {
    email
        .chars()
        .map(|c| {
...
pub fn profile_for_email(email: &str) -> Option<PathBuf> {
    let seeded_suffix = format!("-{}", slugify_email(email));
...
/// Claude Code's project-transcript directory name for a cwd: it replaces
/// every non-alphanumeric character with `-`
/// (`e.replace(/[^a-zA-Z0-9]/g,"-")`). Getting this wrong points callers at a
/// path that does not exist, which must never be read as "nothing happening
/// here" — see `watch::transcript_idle`.
...
#[cfg(test)]
mod tests {
    use super::{project_slug, slugify_email};

    #[test]
    fn slugs_match_the_tools_that_own_them() {
        // cswap: only [alnum._-] survives.
        assert_eq!(slugify_email("a.b@x.com"), "a.b_x.com");
        assert_eq!(slugify_email("clement+work@x.com"), "clement_work_x.com");
        // Claude Code: EVERY non-alphanumeric becomes '-'.
        assert_eq!(
            project_slug("/home/u/my_project/.venv"),
            "-home-u-my-project--venv"
        );
        assert_eq!(project_slug("/tmp/a b+c"), "-tmp-a-b-c");
    }
}
