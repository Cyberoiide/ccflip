...
pub fn go_bedrock(reason: &str, args: &[OsString], mapped_email: Option<&str>) -> ! {
    ...
    match fs::read_to_string(&path) {
        Ok(text) => {
            ...
            exec_bedrock(&text, args, mapped_email, Forced::No)
        }
        Err(_) => {
            eprintln!(
...
pub fn force_bedrock(args: &[OsString]) -> ! {
    let path = bedrock_env_path();
    match fs::read_to_string(&path) {
        // FORCED marks the session as the user's own choice: the watcher's
        // switchback must not undo it the moment quota frees up.
        Ok(text) => exec_bedrock(&text, args, None, Forced::Yes),
        Err(_) => {
...
/// Did the user pick this tier, or did policy?
enum Forced {
    Yes,
    No,
}
...
fn exec_bedrock(
    ...
    if matches!(forced, Forced::Yes) {
        cmd.env("CCFLIP_FORCED_TIER", "bedrock");
    }
