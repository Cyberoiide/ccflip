...
pub struct ClaudeProc {
    ...
    pub age: Duration,
    /// This session asked not to be moved: `CCFLIP_FORCED_TIER` (the user
    /// picked the tier with `ccflip bedrock`) or `CCFLIP_NO_TIER_SWITCH` (a
    /// batch caller that will not be respawned).
    pub opted_out: bool,
}
        ...
        .filter_map(|entry| {
            ...
            for var in environ.split(|b| *b == 0) {
                if let Some(v) = var.strip_prefix(b"CLAUDE_CODE_USE_BEDROCK=") {
                    ...
                    .strip_prefix(b"CCFLIP_FORCED_TIER=")
                    .or_else(|| var.strip_prefix(b"CCFLIP_NO_TIER_SWITCH="))
                {
                    opted_out = opted_out || !v.is_empty();
                }
            }
            Some(ClaudeProc {
                pid,
