...
"#,
    ...
    let run_pwd = |cwd: &Path, pwd: &Path, stale_bedrock: bool, args: &[&str]| {
        // Truncate first: without this a case whose expectation matches the
        // previous case's output passes on a stale file even if ccflip never
        // reached either fake.
        let _ = fs::remove_file(t.join("out"));
